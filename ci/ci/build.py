...
set +x
USERNAME=$(cat /secrets/registry-push-credentials/credentials.json | jq '.appId')
PASSWORD=$(cat /secrets/registry-push-credentials/credentials.json | jq '.password')
az acr login --name {shq(DOCKER_PREFIX)} -u $USERNAME -p $PASSWORD
set -x

until az acr repository delete --yes -n {shq(DOCKER_PREFIX)} --image {shq(self.image)} || ! az acr repository show -n {shq(DOCKER_PREFIX)} --image {shq(self.image)}
do
